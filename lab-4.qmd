...
# Belief that Vaccines are Safe Data Prep
wgm_agree <- wgm_clean %>%
  filter(
...
datatable_with_region <- q19_geo %>%
  mutate(
    ...
    Region = ifelse(is.na(Region), "Other/Unclassified", Region),
    Net = Increase - Decrease  # <-- Add this line to create Net again
  ) %>%
...
output$region_extremes_text <- renderUI({
  ...
  htmltools::tagList(
    lapply(1:nrow(summaries), function(i) {
      ...
      div(
        style = "margin-bottom: 1em; font-size: 13px;",
        strong(region_info$Region), br(),
        paste0("Highest Net: ", region_info$HighestCountry, " (", round(region_info$HighestScore * 100, 1), "%)"), br(),
        paste0("Lowest Net: ", region_info$LowestCountry, " (", round(region_info$LowestScore * 100, 1), "%)")
      )
    })
  )
})
